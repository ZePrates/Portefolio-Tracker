-- Exposição da carteira: perfis de ativos, exposições, holdings de ETFs e
-- cache global de perfis de instrumentos.
-- 100% idempotente: seguro correr numa base vazia OU numa base onde parte
-- (ou a totalidade) destes objetos já existe.
...
CREATE TABLE IF NOT EXISTS public.asset_profiles (
  ...
  CONSTRAINT asset_profiles_asset_id_key UNIQUE (asset_id)
);
...
CREATE POLICY "Users manage their own asset profiles" ON public.asset_profiles
  FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP TRIGGER IF EXISTS update_asset_profiles_updated_at ON public.asset_profiles;
CREATE TRIGGER update_asset_profiles_updated_at BEFORE UPDATE ON public.asset_profiles
...
CREATE TABLE IF NOT EXISTS public.asset_exposures (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT asset_exposures_asset_id_dimension_value_as_of_date_key
...
CREATE POLICY "Users manage their own asset exposures" ON public.asset_exposures
  FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP TRIGGER IF EXISTS update_asset_exposures_updated_at ON public.asset_exposures;
CREATE TRIGGER update_asset_exposures_updated_at BEFORE UPDATE ON public.asset_exposures
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE TABLE IF NOT EXISTS public.etf_holdings (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT etf_holdings_asset_id_holding_name_as_of_date_key
...
CREATE POLICY "Users manage their own etf holdings" ON public.etf_holdings
  FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP TRIGGER IF EXISTS update_etf_holdings_updated_at ON public.etf_holdings;
CREATE TRIGGER update_etf_holdings_updated_at BEFORE UPDATE ON public.etf_holdings
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
-- ================================================= security_profiles ====
-- Tabela global de referência/cache (não pertence a um utilizador).
-- Leitura: qualquer utilizador autenticado. Escrita: apenas server-side /
-- service role (ver 20260911170000_harden_security_profiles.sql).
...
CREATE POLICY "Authenticated can read security profiles" ON public.security_profiles
  FOR SELECT TO authenticated USING (true);

DROP TRIGGER IF EXISTS update_security_profiles_updated_at ON public.security_profiles;
CREATE TRIGGER update_security_profiles_updated_at BEFORE UPDATE ON public.security_profiles
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
